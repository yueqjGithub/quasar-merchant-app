...
export default {
  ...
  methods: {
    toAna () { // 跳转核销详情
      ...
      this.$router.push({name: 'Analysis', params: {type: 1}}) // type 1- 商户进入，2-门店进入
    },
    toCus () {
      this.$store.commit('common/isShowBack', true)
      this.$router.push({name: 'Customer', params: {type: 1}}) // type 1- 商户进入，2-门店进入
    },
    toSettle () {
      this.$store.commit('common/isShowBack', true)
      this.$store.commit('common/setSearchOptions', 2)
      this.$router.push({name: 'Settlement', params: {type: 1}}) // type 1- 商户进入，2-门店进入
    },
    queryInfo (done) {
      let vm = this
      let status = this.status
      let id = this.$q.localStorage.getItem('userInfo').companyid
      let params = {
        'time_status': status,
        'companyid': id
      }
      vm.showLoading = true
      // console.log(params)
      this.$axios(urls.getCompanyInfo, params).then(res => {
        let code = res.code
        if (code === 'success') {
